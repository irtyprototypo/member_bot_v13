...
const { payout, shuffle, escapeHtml } = require('../util.js');
...
module.exports = {
	...
	async execute(interaction) {
		now = interaction.options.getBoolean('now');
		...
		prize = possiblePrizes[DIFF_CHOICES.indexOf(Math.floor(Math.random() * possiblePrizes.length))];
		...
		Promise.resolve(post)
			.then(p => {
				...
				const collection = p.createReactionCollector({
					filter: reaction => {
						return DIFF_CHOICES.includes(reaction.emoji.name)
					...
					if (!user.bot && reaction.emoji.name == GO_EMOJI) {
						switch (phase) {
							...
							case 'question':
								...
								catID = (catID) ? catID : Math.floor(Math.random() * cats.length);
								if (catID == 39) catID = 9;
								...
								triv.getQuestions(questionOptions)
									.then(r => {
										embededResponse.fields.push({ name: `Question [${user.username.split(' ')[0]}]`, value: `${escapeHtml(r.results[0].question)}`, inline: false })
										...
										embededResponse.fields.push({ name: `Choices`, value: `${escapeHtml(answers.toString()).replaceAll(',', '\n')}`, inline: true })
										p.edit({ embeds: [embededResponse] });
